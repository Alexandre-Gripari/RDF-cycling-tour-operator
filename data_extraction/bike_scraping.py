...
for g in [g_bikes, g_clients, g_reviews]:
    g.bind("cs", CS)
    g.bind("cto", CTO_DATA)
    g.bind("foaf", FOAF)
    g.bind("xsd", XSD)
...
def fetch_real_reviews_via_api(scraper, sku, bike_uri, bike_slug):
    ...
    for i, review in enumerate(reviews):
        ...
        raw_comment = review.get('comment') or review.get('title') or "Pas de commentaire."
        raw_rating = review.get('rating', {}).get('code', 5)
        
        author_slug = generate_slug(raw_author)
        client_id = f"{author_slug}_{bike_slug}_{i}"
        review_id = f"{bike_slug}_R{i}"
        
        client_uri = CTO_DATA[f"Client_{client_id}"]
        review_uri = CTO_DATA[f"Review_{review_id}"]

        g_clients.add((client_uri, RDF.type, CS.Client))
        g_clients.add((client_uri, FOAF.name, Literal(raw_author, datatype=XSD.string)))
        g_clients.add((client_uri, CS.phone, Literal(fake.phone_number(), datatype=XSD.string)))
        status = random.choice(["Finished", "Finished", "In Progress"])
        g_clients.add((client_uri, CS.tourStatus, Literal(status, datatype=XSD.string)))
        g_clients.add((client_uri, CS.bike, bike_uri))

        g_reviews.add((review_uri, RDF.type, CS.Review))
        g_reviews.add((review_uri, CS.rating, Literal(float(raw_rating), datatype=XSD.decimal)))
        g_reviews.add((review_uri, CS.reviewText, Literal(raw_comment, datatype=XSD.string)))
...
def scrape_bike_page(url, scraper):
    ...
    g_bikes.add((bike_uri, CS.pricePerDayBike, Literal(rental_price, datatype=XSD.decimal)))
...
def get_bike_links(main_url, scraper):
    ...
    for a in all_links:
        ...
        if '/p/' in href and '/_/R-p-' in href:
            full_url = "https://www.decathlon.fr" + href if href.startswith("/") else href
...
def main():
    ...
    g_bikes.serialize("../cto_data_bikes.ttl", format="turtle")
    print("cto_data_bikes.ttl generated")
    g_clients.serialize("../cto_data_clients.ttl", format="turtle")
    print("cto_data_clients.ttl generated")
    g_reviews.serialize("../cto_data_reviews.ttl", format="turtle")
