def get_sparql_prompt(schema_content, data_summary, user_question):
    ...
    5. **Output**: Output ONLY the SPARQL query inside a Markdown code block (```sparql ... ```). do not add explanations.
    ...
    Assistant:
    ```sparql
    PREFIX cs: [http://data.cyclingtour.fr/schema#](http://data.cyclingtour.fr/schema#)
    PREFIX xsd: [http://www.w3.org/2001/XMLSchema#](http://www.w3.org/2001/XMLSchema#)
    PREFIX rdfs: [http://www.w3.org/2000/01/rdf-schema#](http://www.w3.org/2000/01/rdf-schema#)

    SELECT ?bike ?label ?price WHERE {{
        ?bike a cs:RoadBike ;
              rdfs:label ?label ;
    ...
    SELECT ?clientName (COUNT(?booking) AS ?bookingCount) WHERE {{
        ?booking cs:bookedBy ?client .
